namespace Web_cafe_film.ThuatToan.Implementation
{
    ...
    public class Apriori : IApriori
    {
        ...
        // Lấy về tập thường xuyên (phổ biến)
        ...
        private double GetSupport(string generatedCandidate, IEnumerable<string> transactionsList)
        {
            ...
            {
                if (CheckIsSubsetString(generatedCandidate, transaction))
                {
                    support++;
                }
            }
        ...
        //Viết thêm hàm mới
        private bool CheckIsSubsetString(string child, string parent)
        {
            string[] tmpChild = child.Split(';');
            string[] tmp = parent.Split(';');
            int count = 0;       
            for(int i = 0; i < tmpChild.Length; i++)
                for(int j = 0; j < tmp.Length; j++)
                    if(tmpChild[i].Equals(tmp[j]))
                    {
                        count++;
                        break;
                    }
            if (count == tmpChild.Length)
                return true;
            else
                return false;           
        }
        ...
        private Dictionary<string, double> GenerateCandidates(IList<Item> frequentItems, IEnumerable<string> transactions)
        {
            ...
            {
                string firstItem = frequentItems[i].Name; //_sorter.Sort(frequentItems[i].Name);

                for (int j = i + 1; j < frequentItems.Count; j++)
                {
                    string secondItem = frequentItems[j].Name;//_sorter.Sort(frequentItems[j].Name);
                    string[] tmp = secondItem.Split(';');
                    foreach(string tmpItem in tmp)
                    {
                        if (firstItem.Contains(tmpItem))
                            continue;
                        string generatedCandidate = GenerateCandidate(firstItem, tmpItem);
                        double support = GetSupport(generatedCandidate, transactions);
                        int count = candidates.Where(o => o.Key == generatedCandidate).ToList().Count;
                        if(count < 1)
        ...
        private Dictionary<string, double> GetItemParents(string child, int index, ItemsDictionary allFrequentItems)
        {
            ...
            for (int j = index; j < allFrequentItems.Count; j++)
            {
                ...
                if (CheckIsSubsetString(child, parent))
                {
        ...
        private HashSet<Rule> GenerateRules(ItemsDictionary allFrequentItems)
        {
            ...
            foreach (var item in allFrequentItems)
            {
                ...
                if (tmp.Length > 1)
                {
                    IEnumerable<string> subsetsList = tmp;
        ...
        {
            //Xóa phần tử con trong chuỗi cha để gán vào remain
            string[] tmp = parent.Split(';');
            string str="";
        ...
        private double GetConfidence(string X, string XY, ItemsDictionary allFrequentItems)
        {
            ...
            try
            {
                supportX = allFrequentItems[X].Support;
                supportXY = allFrequentItems[XY].Support;
            }
